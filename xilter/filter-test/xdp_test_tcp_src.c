...
int xdp_parse_prog(struct xdp_md* ctx){
	...
	// priority 1
	priority++;
	if (tcph && (bpf_ntohs(tcph->source) == 0) ){
		goto out;
	}

	// priority 2
	priority++;
	if (tcph && (bpf_ntohs(tcph->source) == 22) ){
		goto out;
	}

	// priority 3
	priority++;
	if (tcph && (bpf_ntohs(tcph->source) == 80) ){
		goto out;
	}

	// priority 4
	priority++;
	if (tcph && (bpf_ntohs(tcph->source) == 179) ){
		goto out;
	}

	// priority 5
	priority++;
	if (tcph && (bpf_ntohs(tcph->source) == 65535) ){
		goto out;
	}
