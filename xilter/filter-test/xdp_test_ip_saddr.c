...
int xdp_parse_prog(struct xdp_md* ctx){
	...
	__u32 ip_saddr1 = 0x000200c0;
	__u32 ip_saddr2 = 0xfe0200c0;
	__u32 ip_saddr3 = 0x0a6433c6;
	__u32 ip_saddr4 = 0x646433c6;
	__u32 ip_saddr5 = 0x017100cb;

	// priority 1
	priority++;
	if (iph->saddr == ip_saddr1) {
		goto out;
	}

	// priority 2
	priority++;
	if (iph->saddr == ip_saddr2) {
		goto out;
	}

	// priority 3
	priority++;
	if (iph->saddr == ip_saddr3) {
		goto out;
	}

	// priority 4
	priority++;
	if (iph->saddr == ip_saddr4) {
		goto out;
	}

	// priority 5
	priority++;
	if (iph->saddr == ip_saddr5) {
		goto out;
	}
