...
int xdp_parse_prog(struct xdp_md* ctx){
	...
	// priority 1
	priority++;
	if (udph && (bpf_ntohs(udph->dest) == 0) ){
		goto out;
	}

	// priority 2
	priority++;
	if (udph && (bpf_ntohs(udph->dest) == 22) ){
		goto out;
	}

	// priority 3
	priority++;
	if (udph && (bpf_ntohs(udph->dest) == 80) ){
		goto out;
	}

	// priority 4
	priority++;
	if (udph && (bpf_ntohs(udph->dest) == 179) ){
		goto out;
	}

	// priority 5
	priority++;
	if (udph && (bpf_ntohs(udph->dest) == 65535) ){
		goto out;
	}
