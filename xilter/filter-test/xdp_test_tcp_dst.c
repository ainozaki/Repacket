...
int xdp_parse_prog(struct xdp_md* ctx){
	...
	// priority 1
	priority++;
	if (tcph && (bpf_ntohs(tcph->dest) == 0) ){
		goto out;
	}

	// priority 2
	priority++;
	if (tcph && (bpf_ntohs(tcph->dest) == 22) ){
		goto out;
	}

	// priority 3
	priority++;
	if (tcph && (bpf_ntohs(tcph->dest) == 80) ){
		goto out;
	}

	// priority 4
	priority++;
	if (tcph && (bpf_ntohs(tcph->dest) == 179) ){
		goto out;
	}

	// priority 5
	priority++;
	if (tcph && (bpf_ntohs(tcph->dest) == 65535) ){
		goto out;
	}
